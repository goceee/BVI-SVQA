...
import { getDirectories, checkFile } from '../utils/fileSystemUtils';
import {
...
import { addTitleBarFunctionality, csvToObject } from '../utils/commonUtils';
import { disableElements } from '../utils/window/dataAnalysisWindowUtils';

let QParr: string[] = [];
let vidCodecsList: string[] = [];
...
comparePlotsButton.onclick = () => {
  ...
  let rateParameter: { type: string; data: string[] } = { type: '', data: [] };
  const selectedVideo = videosList.value;
