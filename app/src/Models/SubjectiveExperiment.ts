...
class SubjectiveExperiment {
  ...
  constructor(experiment: any, scaleType = 'Continuous') {
    ...
    this.#scaleNames = Array.from(
      new Set(
        experiment.experimentOrder
          .filter((element: string) => element.includes('VOTE'))
          .map((element: string) => element.split('_')[1]),
      ),
    );
  }
  ...
  create() {
    ...
    this.#buttons.forEach((buttonText: string) => {
  ...
  // eslint-disable-next-line class-methods-use-this
  stop(reason: string, trial: boolean) {
    let titleText = '';
    if (reason === 'break')
      titleText =
  ...
  scoredVideoNumber(videoPair: string[]) {
...
export default SubjectiveExperiment;
